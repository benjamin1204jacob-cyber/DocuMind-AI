...
import type { Profile, Analysis } from './lib/supabase';
...
const Button = ({ className, variant = 'primary', ...props }: any) => {
  const variants: any = {
    ...
    outline: 'border-2 border-gray-200 hover:border-blue-600 hover:text-blue-600 dark:border-gray-700 dark:hover:border-blue-400 dark:hover:text-blue-400 dark:text-white'
  };
  return (
    <button
      className={cn(
          ...
          >
            <div className="p-8 prose prose-blue dark:prose-invert max-w-none">
              ...
              <div className="mt-8 pt-8 border-t border-gray-100 dark:border-gray-800 flex justify-end">
                <Button onClick={copyToClipboard} variant="secondary" className="text-sm h-10 px-6">
                  {copied ? (
      ...
      <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="w-full max-w-md">
        <div className="text-center mb-8">
          ...
          <p className="text-gray-500 dark:text-gray-400 mt-2">
            {isLogin ? 'Sign in to continue to DocuMind AI' : 'Join thousands of professionals today'}
          </p>
        </div>
        ...
        <Card className="p-8">
          <AnimatePresence mode="wait">
            {error && (
              <motion.div
                ...
                exit={{ opacity: 0, y: -8 }}
                className="flex items-center gap-2 p-3 bg-red-50 dark:bg-red-900/20 text-red-600 dark:text-red-400 rounded-xl mb-4 text-sm"
              >
            ...
            {successMsg && (
              <motion.div
                ...
                animate={{ opacity: 1, y: 0 }}
                exit={{ opacity: 0, y: -8 }}
                className="flex items-center gap-2 p-3 bg-green-50 dark:bg-green-900/20 text-green-600 dark:text-green-400 rounded-xl mb-4 text-sm"
              >
          ...
          <div className="relative mb-6">
            ...
            <div className="relative flex justify-center text-xs uppercase">
              <span className="bg-white dark:bg-gray-900 px-2 text-gray-400 dark:text-gray-500 font-medium">Or with email</span>
            </div>
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <Button type="submit" disabled={loading || googleLoading} className="w-full h-12 text-lg mt-2">
        ...
        <button onClick={onBack} className="w-full mt-4 text-sm text-gray-400 hover:text-gray-600 dark:hover:text-gray-300 transition-colors">
          Back to Home
        </button>
      </motion.div>
    </div>
...
export default function App() {
  ...
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (invalidFiles.length > 0) {
      alert(`Invalid file type(s): ${invalidFiles.map((f) => f.name).join(', ')}. Supported: PDF, Images, Text, JSON, Code files.`);
    }
  ...
  const handleAnalyze = async () => {
    ...
    try {
      ...
      const response = await genAI.models.generateContent({
        ...
        contents: { parts: [...parts, { text: question }] },
  ...
  if (!user) {
    return (
      <div className="min-h-screen bg-white dark:bg-gray-950 text-gray-900 dark:text-gray-100 font-sans selection:bg-blue-100 dark:selection:bg-blue-900/30 transition-colors duration-300">
        <AnimatePresence>
          {showAuth && <AuthScreen onBack={() => setShowAuth(false)} />}
        </AnimatePresence>
        ...
        <nav className="fixed top-0 w-full z-50 bg-white/80 dark:bg-gray-900/80 backdrop-blur-md border-b border-gray-100 dark:border-gray-800 transition-colors">
          <div className="max-w-7xl mx-auto px-6 h-20 flex items-center justify-between">
            ...
            <div className="hidden md:flex items-center gap-8">
              ...
              <Button onClick={() => setShowAuth(true)} className="text-sm">Get Started</Button>
            </div>
          ...
          {isMenuOpen && (
            ...
            >
              <div className="p-6 flex flex-col gap-4">
                <a href="#features" onClick={() => setIsMenuOpen(false)} className="text-lg font-medium text-gray-600 dark:text-gray-300 hover:text-blue-600 dark:hover:text-blue-400 transition-colors">
                  Features
                </a>
                <Button onClick={() => { setShowAuth(true); setIsMenuOpen(false); }} className="w-full h-12">
                  Get Started
                </Button>
              </div>
            </motion.div>
          ...
          <div className="max-w-4xl mx-auto text-center">
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }}>
              <div className="inline-flex items-center gap-2 px-4 py-1.5 bg-blue-50 dark:bg-blue-900/20 text-blue-600 dark:text-blue-400 rounded-full text-xs font-bold uppercase tracking-wider mb-6">
                ...
                Beta Early Access - Powered by Gemini 2.5 Pro
              </div>
              ...
              <p className="text-xl text-gray-500 dark:text-gray-400 mb-10 max-w-2xl mx-auto leading-relaxed">
                Upload PDFs, images, or code and get instant answers, summaries, and insights. The most advanced AI document analysis tool for professionals.
              </p>
              <div className="flex flex-col sm:flex-row items-center justify-center gap-4 w-full">
                <Button onClick={() => setShowAuth(true)} className="w-full sm:w-auto h-14 px-10 text-lg">
          ...
          <div className="max-w-7xl mx-auto px-6">
            <div className="grid md:grid-cols-3 gap-8">
              {[
                ...
                { icon: Brain, title: 'Deep Reasoning', desc: 'Powered by Gemini 2.5 Pro for unmatched comprehension.' }
              ].map((f, i) => (
      ...
      <header className="bg-white dark:bg-gray-900 border-b border-gray-200 dark:border-gray-800 sticky top-0 z-50 transition-colors">
        <div className="max-w-7xl mx-auto px-6 h-16 flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <span className="font-bold text-lg tracking-tight dark:text-white">DocuMind</span>
          </div>
          <div className="flex items-center gap-3">
            ...
            {profile?.photo_url || user.user_metadata?.avatar_url ? (
              ...
              <div className="w-8 h-8 rounded-full border border-gray-200 dark:border-gray-700 bg-blue-100 dark:bg-blue-900/30 flex items-center justify-center text-blue-600 dark:text-blue-400 text-sm font-bold">
          ...
          <Card className="p-0 overflow-hidden border-dashed border-2 border-gray-200 dark:border-gray-800 bg-gray-50/50 dark:bg-gray-900/50">
            {files.length === 0 ? (
              ...
              >
                ...
                <p className="text-sm text-gray-500 dark:text-gray-400 text-center max-w-xs">Supported: PDF, Images, JSON, JS, TS, HTML, CSS (Max 50MB)</p>
                <input
                  type="file"
                  ref={fileInputRef}
                      ...
                      <div className="flex items-center justify-between mb-2">
                        <div className="flex items-center gap-3 min-w-0">
                          <div className="w-10 h-10 bg-blue-50 dark:bg-blue-900/30 rounded-lg flex items-center justify-center text-blue-600 dark:text-blue-400 flex-shrink-0">
                            ...
                            ) : f.name.endsWith('.ts') || f.name.endsWith('.tsx') || f.name.endsWith('.js') ? (
                        ...
                        <button onClick={() => removeFile(i)} className="p-2 text-gray-400 hover:text-red-500 transition-colors">
                          <X className="w-4 h-4" />
                        </button>
                      </div>
                      {preview && (
                        <div className="mt-2 p-2 bg-white dark:bg-gray-950 rounded-xl border border-gray-100 dark:border-gray-800">
                          <p className="text-[10px] font-mono text-gray-500 dark:text-gray-400 line-clamp-3 break-all">{preview}</p>
                        </div>
                      )}
                    </div>
                  ))}
          ...
          {files.length > 0 && (
            <div className="flex items-center justify-between px-4 py-3 bg-blue-50/50 dark:bg-blue-900/10 rounded-2xl border border-blue-100 dark:border-blue-900/30">
              ...
              <button
                ...
                className={cn('w-12 h-6 rounded-full transition-colors relative', highThinking ? 'bg-blue-600' : 'bg-gray-300 dark:bg-gray-700')}
              >
                <div className={cn('absolute top-1 left-1 w-4 h-4 bg-white rounded-full transition-transform', highThinking ? 'translate-x-6' : 'translate-x-0')} />
              </button>
            </div>
          )}
            ...
            <div className="space-y-6">
              <div className="relative">
                ...
                <Button
                  ...
                  className={cn(
                    'absolute bottom-4 right-4 h-12 transition-all duration-500 ease-out shadow-lg active:scale-95 group',
                    analyzing
                      ? 'w-40 rounded-2xl bg-gray-100 dark:bg-gray-800 text-gray-400 border border-gray-200 dark:border-gray-700'
                      : 'w-auto px-6 rounded-2xl bg-gradient-to-r from-blue-600 to-blue-700 hover:from-blue-700 hover:to-blue-800 text-white hover:shadow-blue-200 dark:hover:shadow-blue-900/20'
                  )}
                >
                  {analyzing ? (
                    <><Loader2 className="w-4 h-4 animate-spin" /><span className="text-sm font-bold tracking-tight">Processing...</span></>
                  ) : (
                    <><span className="text-sm font-bold tracking-tight">Analyze Now</span><Sparkles className="w-4 h-4 animate-pulse" /><ArrowRight className="w-4 h-4 group-hover:translate-x-1 transition-transform" /></>
                  )}
                </Button>
              </div>
        ...
        <div className="space-y-8">
          ...
          <div className="space-y-4">
            <div className="flex items-center justify-between px-2">
              ...
              {history.length > 0 && (
                <button onClick={clearHistory} className="text-[10px] font-bold text-red-500 hover:text-red-600 uppercase tracking-wider transition-colors">
                  Clear All
                </button>
              )}
            </div>
              ...
              {history.length === 0 ? (
                ...
                history.slice(0, 5).map((item) => (
                  <div
                    ...
                    onClick={() => {
                      setFiles([{ file: new File([], item.file_name.split(', ')[0]) }]);
                  ...
                  >
                    <div className="flex items-center gap-3 mb-2">
                      ...
                      <span className="text-sm font-bold truncate group-hover:text-blue-600 dark:group-hover:text-blue-400 dark:text-gray-200">{item.file_name}</span>
                    </div>
